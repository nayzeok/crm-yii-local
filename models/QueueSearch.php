...
class QueueSearch extends Queue
{
    ...
    {
        return [
            [['id', 'priority', 'attempts', 'interval'], 'integer'],
            [['name', 'created_at', 'update_at', 'delete_at'], 'safe'],
        ];
    }
    ...
    public function search(array $params): ActiveDataProvider
    {
        ...
        if (!empty($this->created_at)) {
            if (preg_match('/^\d{4}$/', $this->created_at)) {
                // Фильтрация по году
                $query->andFilterWhere(['YEAR(created_at)' => $this->created_at]);
            } elseif (preg_match('/^\d{4}-\d{2}$/', $this->created_at)) {
                // Фильтрация по году и месяцу
                $query->andFilterWhere(['like', 'DATE_FORMAT(created_at, "%Y-%m")', $this->created_at]);
            } elseif (preg_match('/^\d{4}-\d{2}-\d{2}$/', $this->created_at)) {
                // Фильтрация по конкретной дате
                $query->andFilterWhere(['like', 'DATE_FORMAT(created_at, "%Y-%m-%d")', $this->created_at]);
            }
        }
