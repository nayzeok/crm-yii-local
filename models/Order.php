...
class Order extends ActiveRecord
{
            ...
            if ($insert || (isset($changedAttributes['status']) && $changedAttributes['status'] != $this->status)) {
                ...
                foreach ($triggers as $trigger) {
                    if ($trigger->trigger_type == Trigger::TRIGGER_TYPE_LEAD_WITH_WEB_ID_OR_SITE) {
                        // Проверка соответствия значений lead_web_id и lead_site, учитывая null
                        if (($this->lead_web_id !== null && $trigger->lead_web_id == $this->lead_web_id) ||
                            ($this->lead_site !== null && $trigger->lead_site == $this->lead_site) ||
                            ($this->campaign_id !== null && $trigger->campaign_id == $this->campaign_id)) {
                            $this->blocked_at = null;
                            $this->link('currentQueue', $trigger->queue);
                        }
                    } else if ($trigger->trigger_type == Trigger::TRIGGER_TYPE_LEAD) {
                        // Логика для обработки обычного триггера без специфических параметров
                ...
                switch ($trigger->entity) {
                    case Trigger::ACTION_TYPE_QUEUE:
                        if ($trigger->entity_id != $this->current_queue_id) {
                            ...
                            $this->blocked_at = null;
                            $this->save(true, ['current_queue_id', 'blocked_at']);
                        }
                        break;
    ...
    {
        return [
            [['customer_name', 'customer_phone', 'status'], 'required'],
            ...
            [['customer_name', 'customer_phone', 'lead_site', 'lead_partner_id'], 'string', 'max' => 255],
    ...
    public function getPreparedAPIData(): ?array
    {
        return [
            ...
            "customer_mobile" => $this->customerInfo->mobile,
